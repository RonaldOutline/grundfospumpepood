...
export default function KategooriaPage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  const loadProducts = useCallback(async () => {
    ...
    // Kogu kõik slugid: praegune + kõik alamkategooriad
    const { data: allCats } = await supabase
      .from('categories')
    ...
    // Leia product_id-d kõigi slugide järgi
      ...
      <div className="max-w-7xl mx-auto px-4 py-6">
        ...
        <div className="flex items-center gap-3 mb-5 flex-wrap">
          <div className="relative flex-1 min-w-[200px]">
            ...
            <input type="search" placeholder="Otsi selles kategoorias..."
          ...
          <div className="flex flex-wrap items-center gap-2 mb-5">
            ...
            {inStockOnly && (
              <span className="flex items-center gap-1.5 bg-green-50 text-green-700 text-[15px] font-medium px-3 py-1 rounded-full">
                Laos olemas <button onClick={() => { setInStockOnly(false); updateUrl({ laos: '' }) }}><X size={13} /></button>
              </span>
            )}
            {(priceMin || priceMax) && (
            ...
            <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-5 sticky top-24">
              ...
              <div>
                ...
                <label className="flex items-center gap-3 cursor-pointer">
                  ...
                  <span className="text-[15px] text-gray-700">Ainult laos</span>
                </label>
              </div>
            </div>
          </aside>
